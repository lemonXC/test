<template>
  <div class="user-issue-management">
    ...
    <button
      class="fab-add-issue"
      @click="onAddIssueClick"
      @keydown="onAddIssueKeydown"
      role="button"
      aria-label="添加问题"
      aria-describedby="fab-add-issue-tip"
      title="添加问题"
      tabindex="0"
    >
      <van-icon name="plus" />
    </button>
    <div id="fab-add-issue-tip" class="fab-tooltip" aria-hidden="true">添加问题</div>
  </div>
</template>
...
export default {
  ...
  setup() {
    ...
    // 动态页面标题计算属性
    // 根据路由查询参数source来判断页面标题
    const pageTitle = computed(() => {
      const source = route.query.source
      switch (source) {
        case 'submit_issue':
          return '上报问题'  // 从"信息上报"入口进入
        case 'my_issues':
          return '我的诉求'  // 从"我的问题"入口进入
        default:
          return '我的问题'  // 默认标题，兼容直接访问的情况
      }
    })
    ...
    watch(
      () => route.query.source,
      (newSource, oldSource) => {
        // 当source参数发生变化时，pageTitle会自动重新计算
        // 这里可以添加额外的逻辑，比如日志记录
        console.log(`页面标题更新: ${oldSource} -> ${newSource}`)
      },
      { immediate: true }
    )
    ...
    return {
      // 悬浮添加按钮
      onAddIssueClick: () => router.push({ name: 'submit-issue' }),
      onAddIssueKeydown: (e) => {
        if (e.key === 'Enter' || e.key === ' ') {
          e.preventDefault()
          router.push({ name: 'submit-issue' })
        }
      },
      pageTitle,
      activeTab,
...
.fab-add-issue:active {
  transform: scale(0.95);
}
.fab-add-issue:focus {
  outline: none;
  box-shadow: 0 0 0 3px rgba(24, 144, 255, 0.3), 0 2px 10px rgba(0, 0, 0, 0.2);
}
.fab-add-issue:hover {
  background-color: #1677ff;
}
.fab-add-issue .van-icon {
  font-size: 28px;
}
...
.fab-add-issue:hover + .fab-tooltip,
.fab-add-issue:focus + .fab-tooltip {
  opacity: 1;
